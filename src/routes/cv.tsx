  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="text-xs font-mono text-primary uppercase tracking-[0.2em]">{t("cv_kicker")}</div>
          <h1 className="mt-3 font-display text-4xl md:text-5xl font-bold">Ismail Chekalil</h1>
          <div className="mt-2 text-muted-foreground">{t("cv_role")}</div>
        </div>
      ...
      <div className="mt-14 grid md:grid-cols-12 gap-12">
        <div className="md:col-span-8 space-y-12">
          <div>
            <h2 className="font-display text-2xl font-bold">{t("cv_experience")}</h2>
            <div className="mt-6 space-y-8">
              {experience.map((e) => (
                <div key={e.role} className="relative ps-6 border-s border-border">
                  <span className="absolute -start-[5px] top-2 h-2.5 w-2.5 rounded-full bg-primary" />
                  ...
                  <ul className="mt-3 space-y-1.5 text-sm text-muted-foreground list-disc ps-4">
          ...
          <div>
            <h2 className="font-display text-2xl font-bold">{t("cv_education")}</h2>
        ...
        <aside className="md:col-span-4 space-y-8">
          <div className="p-6 rounded-xl border border-border bg-card">
            <h3 className="font-display font-semibold">{t("cv_contact_h")}</h3>
          ...
          <div className="p-6 rounded-xl border border-border bg-card">
            <h3 className="font-display font-semibold">{t("cv_certs_h")}</h3>
            <ul className="mt-3 text-sm text-muted-foreground space-y-2">
              {certs.map((c) => <li key={c} className="flex gap-2"><span className="text-primary">›</span>{c}</li>)}
            </ul>
          </div>
          <div className="p-6 rounded-xl border border-border bg-card">
            <h3 className="font-display font-semibold">{t("cv_tools_h")}</h3>
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              {["NX CAM", "Heidenhain", "Siemens 840sl", "SLM", "Meltio DED", "FDM", "Hexagon CMM", "Python / ML"].map((t2) => (
                <span key={t2} className="px-2 py-1 rounded-md border border-border bg-secondary">{t2}</span>
              ))}
            </div>
          </div>
        </aside>
